	...
	tests := map[string]func(t *testing.T) ([]*service.MetricsWrapper, []checkFn){
		"success": func(t *testing.T) ([]*service.MetricsWrapper, []checkFn) {
			...
			getMeter := func(prefix string) *service.MetricsWrapper {
				...
				provisioned, err := otMeter.NewFloat64UpDownCounter(prefix + "thin_savings_megabytes")
				if err != nil {
					t.Fatal(err)
				}

				used, err := otMeter.NewFloat64UpDownCounter(prefix + "max_thin_savings_megabytes")
				if err != nil {
					t.Fatal(err)
				}
